# 🐾 PalSystem — Unreal Engine 5 Multiplayer Project

> Palworld-inspired 멀티플레이어 게임의 Pal 시스템 구현 프로젝트
...
├── PROJECT_TRACKING.md          ← 스프린트 추적 및 태스크 목록
├── LICENSE
└── palworld/                    ← UE5 프로젝트 폴더
    ├── Config/
    ├── Content/
...
main
 └── develop
      ├── feature/OBI           ← 팀원 개인 작업 브랜치 (이니셜)
      ├── feature/combat        ← 특정 모듈 작업 브랜치
      ├── bugfix/pal-health-sync
      └── refactor/pal-character
```
...
# 모듈 브랜치 예시 — 시스템 단위 작업
git checkout -b feature/pal-follow
git checkout -b feature/pal-combat
git checkout -b feature/pal-capture
git checkout -b bugfix/pal-health-replication
```
...
3. 공통 폴더로 복사
   _Private/OBI/ → _Common/ (에셋)
   Private/OBI/  → PalSystem/ (소스)
...
```
feat(pal-character): Add Health Replication
fix(pal-combat): Fix damage not applying on client
docs: Update README
```
...
| [PROJECT_TRACKING.md](./PROJECT_TRACKING.md) | 스프린트 계획 및 전체 태스크 목록 |
