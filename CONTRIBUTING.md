...
main
 └── develop
      ├── feature/pal-follow-system
      ├── feature/pal-combat-system
      ├── bugfix/pal-replication-health
      └── refactor/pal-character-cleanup
```
...
예시:
feature/pal-follow-system
feature/pal-capture-rpc
bugfix/pal-health-replication
refactor/pal-character-base
```
...
```
[FEAT] Pal Follow System 서버 이동 로직 구현
[FEAT] APalCharacter Health Replication 설정
[BUG] 클라이언트에서 Pal HP가 동기화되지 않는 문제
[BUG] PIE 2인 환경에서 Pal AI가 서버에서만 동작하는 문제
```
...
| Type | 설명 | 예시 |
|------|------|------|
| `feat` | 새로운 기능 추가 | `feat(pal): Add Health Replication` |
| `fix` | 버그 수정 | `fix(pal): Fix Health not syncing on client` |
| `refactor` | 리팩토링 | `refactor(pal): Reorganize PalCharacter header` |
| `docs` | 문서 수정 | `docs: Update CONTRIBUTING.md` |
| `style` | 코드 포맷 수정 | `style(pal): Apply coding convention` |
| `perf` | 성능 개선 | `perf(pal): Reduce replication frequency` |
| `test` | 테스트 추가 | `test(pal): Add PIE replication test` |
| `chore` | 빌드, 설정 변경 | `chore: Add .gitignore for UE5` |

### Scope 종류 (Pal System 기준)

```
pal-character    → APalCharacter 관련
pal-ai           → AI Controller, Behavior Tree
pal-follow       → Follow System
pal-combat       → Combat System
pal-capture      → Capture System
pal-spawn        → Spawn System
pal-skill        → Skill System
pal-anim         → Animation
pal-replication  → Replication / RPC
ui               → UI 관련
```
...
```
feat(pal-character): Add Health variable with Replication
...
```
fix(pal-replication): Fix Health not updating on client
...
| `pal-ai` | `#1d76db` | AI 시스템 관련 |
| `pal-replication` | `#0e8a16` | Replication/RPC 관련 |
| `pal-combat` | `#e11d48` | Combat 시스템 관련 |
...
class UPalCombatComponent : public UActorComponent { };
struct FPalStatusData { };
enum class EPalState : uint8 { };
class IPalInteractable { };
```
...
UPROPERTY(ReplicatedUsing = OnRep_PalState)
EPalState CurrentState;

UPROPERTY(EditDefaultsOnly, Category = "Pal|Stats")
...
UPROPERTY(EditDefaultsOnly, Category = "Pal|Stats")
UPROPERTY(EditDefaultsOnly, Category = "Pal|AI")
UPROPERTY(EditDefaultsOnly, Category = "Pal|Combat")
...
void APalCharacter::TakeDamage(float DamageAmount)
{
...
void APalSpawnSystem::SpawnPal(TSubclassOf<APalCharacter> PalClass)
{
    if (!HasAuthority()) return;

    GetWorld()->SpawnActor<APalCharacter>(PalClass, ...);
}

// 규칙 3: RPC Validate 함수는 반드시 구현
bool APalCharacter::ServerRequestAttack_Validate(AActor* TargetActor)
{
...
│   └── BehaviorTree/
├── Combat/
│   ├── PalCombatComponent.h
│   └── PalCombatComponent.cpp
├── Capture/
├── Spawn/
├── Skill/
└── Data/
