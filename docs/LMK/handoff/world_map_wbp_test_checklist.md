...
- `WorldMin`, `WorldMax`: 월드맵 Texture와 맞는 월드 좌표 범위
...
3. `WorldMin`, `WorldMax`, `GridWidth`, `GridHeight`, `RevealRadius`가 월드맵 Texture 기준과 맞는지 확인한다.
...
10. 미방문 지역은 검정 마스크로 남고 방문 지역은 지도 Texture가 보이는지 확인한다.
11. 마우스 휠로 `MapZoomRoot`가 확대/축소되는지 확인한다.
12. 좌클릭 드래그로 확대된 지도가 이동하는지 확인한다.
13. 발견 처리된 `APW_TeleportPointActor`만 텔레포트 마커로 표시되는지 확인한다.
14. 배치된 `APW_BaseCampActor`가 거점 마커로 표시되는지 확인한다.
15. `MakeExplorationSaveData()` 결과를 저장한 뒤 `ApplyExplorationSaveData()`로 방문 셀이 복원되는지 확인한다.
